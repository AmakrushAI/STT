...
for k,m in config.items():
    if eval(m['lm_usage']):
        ...
        generator = W2lKenLMDecoder(lmarg, dictionary, 'cuda')
    else:
...
def align(fp_arr,model,generator,dictionary,cuda='cuda'):
    ...
    hyp_pieces = dictionary.string(hypo[0][0]["tokens"].int().cuda())
